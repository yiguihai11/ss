...
v2ray_core_old=${v2ray_core:?}
now=$(wget -qO- https://api.github.com/repos/v2fly/v2ray-core/commits/master | jq -r '.sha')
if [ "${v2ray_core:=0}" != "${now:=0}" ] && [ "${v2ray_core:=0}" != 0 ]; then
	v2ray_core="$now"
	((i = i + 1))
else
	unset -v v2ray_core
...
tun2socks_old=${tun2socks:?}
now=$(wget -qO- https://api.github.com/repos/xjasonlyu/tun2socks/commits/main | jq -r '.sha')
if [ "${tun2socks:=0}" != "${now:=0}" ] && [ "${tun2socks:=0}" != 0 ]; then
	tun2socks="$now"
	((i = i + 1))
else
	unset -v tun2socks
...
trojan_old=${trojan:?}
now=$(wget -qO- https://api.github.com/repos/trojan-gfw/trojan/commits/master | jq -r '.sha')
if [ "${trojan:=0}" != "${now:=0}" ] && [ "${trojan:=0}" != 0 ]; then
	trojan="$now"
	((i = i + 1))
else
	unset -v trojan
...
data=$(wget -qO- https://api.github.com/repos/shadowsocks/shadowsocks-rust/commits/master)
now=$(echo $data | jq -r '.sha')
if [ "${shadowsocks_rust:=0}" != "${now:=0}" ] && [ "${shadowsocks_rust:=0}" != 0 ]; then
	unset -v shadowsocks_rust
	for j in $(echo $data | jq -r '.files|.[].filename'); do
		if [[ $j == *.rs ]] || [[ $j == Cargo.* ]]; then
...
if [ "${i:=0}" -gt 0 ]; then
	curl -X POST \
		...
		-F "variables[v2ray_core_old]=${v2ray_core_old:=false}" \
		-F "variables[v2ray_core]=${v2ray_core:=false}" \
		-F "variables[tun2socks_old]=${tun2socks_old:=false}" \
		-F "variables[tun2socks]=${tun2socks:=false}" \
		...
		-F "variables[trojan_old]=${trojan_old:=false}" \
		-F "variables[trojan]=${trojan:=false}" \
