...
sort conf/android_list | uniq >/tmp/android_list
mv -vf /tmp/android_list conf/android_list
sed -i "s/${ss_main_old:?}/${ss_main:?}/g" version/version
git add src/manager.sh usr/bin/ss-main version/version conf/android_list
