...
set -e
apt-get -qq update
apt-get --yes install --no-install-recommends curl ca-certificates jq
...
for PIPELINE in $(curl --header "PRIVATE-TOKEN: ${my_access_token:?}" "https://gitlab.com/api/v4/projects/${CI_PROJECT_ID:?}/pipelines?per_page=100&sort=asc" | jq '.[].id'); do
	if [ "$PIPELINE" -ne ${CI_PIPELINE_ID:-0} ]; then
		...
		curl -L -s -q --header "PRIVATE-TOKEN: ${my_access_token:?}" --request "DELETE" "https://gitlab.com/api/v4/projects/${CI_PROJECT_ID:?}/pipelines/$PIPELINE"
	fi
done
