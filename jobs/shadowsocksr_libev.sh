...
make
make install
strip /etc/ssmanager/usr/bin/ss-local
...
mv -vf /etc/ssmanager/usr/bin/ss-local ${CI_PROJECT_DIR}/usr/bin/ssr-local
cd ${CI_PROJECT_DIR:?}
sed -i "s/${shadowsocksr_libev_old:?}/${shadowsocksr_libev:?}/g" version/version
git add usr/bin/ssr-local version/version temp/upgrade.log
