...
if [ "$shadowsocks_rust" != 'false' ] && [ "$shadowsocks_rust_old" ]; then
    $command usr/bin/sslocal usr/bin/ssmanager usr/bin/ssserver usr/bin/ssurl
	((i = i + 1))
fi
EOF
if [ "$shadowsocksr_libev" != 'false' ] && [ "$shadowsocksr_libev_old" ]; then
	$command usr/bin/ssr-local
	((i = i + 1))
fi
if [ "$trojan" != 'false' ] && [ "$trojan_old" ]; then
	$command usr/bin/trojan
	((i = i + 1))
fi
...
if [ "$v2ray_core" != 'false' ] && [ "$v2ray_core_old" ]; then
	$command usr/bin/v2ray
	((i = i + 1))
fi
if [ "$tun2socks" != 'false' ] && [ "$tun2socks_old" ]; then
