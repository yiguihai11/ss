...
get_china_ips() {
	local chv4 chv6
	chv4=$(curl --silent --connect-timeout 5 --location https://bgp.space/china.html | grep -oP '([0-9]+\.){3}[0-9]+?\/[0-9]{1,2}')
	chv6=$(curl --silent --connect-timeout 5 --location https://bgp.space/china6.html | grep -oP '([0-9a-fA-F]{0,4}:){1,7}[0-9a-fA-F]{0,4}\/[0-9]{1,3}')
	if [ "$chv4" ] && [ "$chv6" ]; then
		if [ -s ${HOME_DIR:?}/conf/server_block.acl ]; then
