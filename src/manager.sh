		...
		if [ "$ipv4" ]; then
			...
			if [ -z "$my_ipv4" ]; then
				if [ ${Language:=en-US} = 'zh-CN' ]; then
					...
					Prompt "Failed to get IPv4 address!"
				fi
				Exit
			fi
		fi
		if [ "$ipv6" ]; then
			...
			if [ -z "$my_ipv6" ]; then
				if [ ${Language:=en-US} = 'zh-CN' ]; then
					...
					Prompt "Failed to get IPv6 address!"
				fi
				Exit
			fi
		fi
	...
	while true; do
		...
		if [ ${cs:-0} -eq 0 ]; then
			...
			if [ "$ipv4" ] && [ "$Protocol" = "ipv4" ]; then
				...
				unset -v ipv4 ipv6 internet4 internet6
			fi
...
Check() {
	...
	local az=0 package_list sorted_arr i2
	...
	if [ -z "$URL" ]; then
		...
		if [ "$(curl -s -o /dev/null -w '%{http_code}' --connect-timeout 5 --resolve gitlab.com:443:172.65.251.78 https://gitlab.com/yiguihai/ss/-/raw/dev/README.md)" = 200 ]; then
			test2=0
		else
			test2=1
		fi
	...
	if ! ${python:-python3} -c "import rich" 2>/dev/null; then
		${pip:-pip3} install -q --upgrade pip
...
Add_user() {
	...
	*)
	...
	EOF
	Prompt "Android客户端和插件 https://gitlab.com/yiguihai/ss/-/wikis/客户端下载"
	...
	if [ -s $HOME_DIR/port.list ]; then
		...
		while IFS= read -r line || [ -n "$line" ]; do
			...
			if is_number "$server_port" && is_number $total; then
				...
				if [ "$server_port" -ne "$port" ] && [ "$password" ] && [ "$method" ] && [ $total -ge 0 ]; then
...
Upload_users() {
	if [ -s $HOME_DIR/port.list ]; then
		local using client_file sy sy2 sspid sorted_arr
				...
				if [ -s ${HOME_DIR:?}/port.list ]; then
					...
					while IFS= read -r line || [ -n "$line" ]; do
						...
						if is_number "$server_port" && is_number "$flow" && is_number $total; then
							if [ ${total:-0} -gt 0 ] && [ "${flow:-0}" -ge ${total:-0} ]; then
...
EOF
	${python:-python3} -m rich.json /tmp/ssr-redir.conf
}
...
Reload_nginx() {
	...
	if [ -d /proc/"${ngx:=lzbx}" ]; then
		...
		if [ "${Nginx_Switch:=disable}" = "enable" ]; then
					...
					while true; do
						...
						if [ ${cs:-0} -eq 0 ]; then
							...
							if curl --connect-timeout 1 --max-time 2 -x socks5://127.0.0.1:1080 'https://www.cloudflare.com/cdn-cgi/trace/' 2>/dev/null; then
								(setsid ipt2socks --redirect >/dev/null 2>&1 &)
								Start_traffic_forward
								echo
								warp-cli warp-stats
								break
