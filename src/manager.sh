...
Parsing_User() {
	unset -v server_port password method plugin plugin_opts used_traffic total reset_day reset_type expire_timestamp upload_limit download_limit
	IFS='|'
	for l in $1; do
		case ${l%^*} in
		server_port)
		...
		if [ "$nginx_on" ]; then
			...
			if [ "$my_server_ip" ]; then
				if [ "${my_ipv4:-$my_ipv6}" = "$my_server_ip" ]; then
					...
					Prompt "你的服务器IP(${my_ipv4:-$my_ipv6})与域名解析到的IP(${my_server_ip})不对应！如果使用了Cloudflare的CDN你需要关闭代理状态"
					Exit
				fi
			else
				Prompt "获取IP地址失败！"
...
Add_user() {
	...
	clear
	local userinfo traffic1 timestamp qrv4 qrv6 name plugin_url ss_info=() ss_link=()
	if [ "$my_ipv4" ]; then
		if [ ${Language:=zh-CN} = 'en-US' ]; then
	...
	simple-obfs)
		...
		echo "server_port^$server_port|password^$password|method^$method|plugin^obfs-server|plugin_opts^obfs=$obfs|used_traffic^0|total^$((total * 1048576))|reset_day^$reset_day|reset_type^$reset_type|expire_timestamp^$expire_timestamp|upload_limit^$upload_limit|download_limit^$download_limit" >>$HOME_DIR/port.list
	...
	v2ray-plugin)
		...
		echo "server_port^$server_port|password^$password|method^$method|plugin^v2ray-plugin|plugin_opts^$v2ray_modes|used_traffic^0|total^$((total * 1048576))|reset_day^$reset_day|reset_type^$reset_type|expire_timestamp^$expire_timestamp|upload_limit^$upload_limit|download_limit^$download_limit" >>$HOME_DIR/port.list
	...
	*)
		...
		echo "server_port^$server_port|password^$password|method^$method|plugin^|plugin_opts^|used_traffic^0|total^$((total * 1048576))|reset_day^$reset_day|reset_type^$reset_type|expire_timestamp^$expire_timestamp|upload_limit^$upload_limit|download_limit^$download_limit" >>$HOME_DIR/port.list
		;;
	esac
	Check_permissions
	if [ "$plugin" ]; then
		...
		while IFS= read -r line || [ -n "$line" ]; do
			...
			if is_number "$server_port" && is_number $total; then
				...
				if [ "$server_port" ] && [ "$password" ] && [ "$method" ]; then
					echo "server_port^$server_port|password^$password|method^$method|plugin^$plugin|plugin_opts^$plugin_opts|used_traffic^$used_traffic|total^$total|reset_day^$reset_day|reset_type^$reset_type|expire_timestamp^$expire_timestamp|upload_limit^$upload_limit|download_limit^$download_limit" >>"$temp_file"
				fi
			fi
		done <$HOME_DIR/port.list
				...
				if [ "$1" != "all" ] && [ -z "$port" ]; then
					...
					if [ "$1" = "check" ]; then
						if is_number "$reset_day" && [ "$reset_day" -gt 0 ] && [ "$reset_type" ]; then
							if [ "$reset_type" = "specify" ] && [ "$(date +'%d')" = "$reset_day" ]; then
								used_traffic=0
								array4+=("$server_port")
							fi
...
Daemon() {
	...
	if [ -r /run/ss-manager.pid ] && [ -r /run/ss-daemon.pid ]; then
		...
		if is_number "$pid1" && is_number "$pid2"; then
			while [ -d /proc/"${pid1:?}" ] && [ -d /proc/"${pid2:?}" ] && [ -S /tmp/ss-manager.socket ]; do
...
V2ray_Link_Decode() {
	local link a alterId host id network path scy port servername tls
	read -rp "请输入vmess://链接: " link
	[[ $link != "vmess://"* || -z $link ]] || {
		a=${link#vmess\:\/\/}
		a=$(echo "$a" | base64 -d 2>&-)
		server=$(echo "$a" | jq -er 'select(.add != null)|.add')
		alterId=$(echo "$a" | jq -er 'select(.aid != null)|.aid')
		host=$(echo "$a" | jq -er 'select(.host != null)|.host')
		id=$(echo "$a" | jq -er 'select(.id != null)|.id')
		network=$(echo "$a" | jq -er 'select(.net != null)|.net')
		path=$(echo "$a" | jq -er 'select(.path != null)|.path')
		scy=$(echo "$a" | jq -er 'select(.scy != null)|.scy')
		port=$(echo "$a" | jq -er 'select(.port != null)|.port')
		servername=$(echo "$a" | jq -er 'select(.sni != null)|.sni')
		tls=$(echo "$a" | jq -er 'select(.tls != null)|.tls')
		if is_number "$port" && [ "$port" -gt 0 ] && [ "$port" -le 65535 ]; then
...
Start_tun2socks() {
	(setsid tun2socks --device tun://tun0 --proxy "$1" -loglevel silent -interface "$internet4" >/dev/null 2>&1 &)
	Tun_Check
...
Proxy_Check() {
	...
	local check_tcp=0 check_udp=0 ip_addr #check_google=0
...
Start_traffic_forward_v6() {
	...
	ip6tables -w -t mangle -A SHADOWSOCKS_v6 -p tcp -m set --match-set traffic_forward_v6 dst -j MARK
...
kernel_install() {
	...
	if [ "$kernel_ver" != "5.13.12" ]; then
		...
		local dl=("${URL:?}/backups/linux-headers-5.13.12_5.13.12-1_amd64.deb+/tmp/linux-header.deb")
		dl+=("${URL:?}/backups/linux-image-5.13.12_5.13.12-1_amd64.deb+/tmp/linux-image.deb")
		dl+=("${URL:?}/backups/linux-libc-dev_5.13.12-1_amd64.deb+/tmp/linux-libc.deb")
		if [ -L "$(find / -type l -name 'libxtables.so.11')" ]; then
			dl+=("${URL:?}/backups/libxt_tls_old.so+/tmp/libxt_tls.so")
			dl+=("${URL:?}/backups/xt_tls_old.ko+/tmp/xt_tls.ko")
		else
			dl+=("${URL:?}/backups/libxt_tls.so+/tmp/libxt_tls.so")
			dl+=("${URL:?}/backups/xt_tls.ko+/tmp/xt_tls.ko")
		fi
		Downloader "${dl[@]}"
...
19. SNI阻断屏蔽
EOF
		read -rp $'请选择 \e[95m1-19\e[0m: ' -n2 action
		echo
		case $action in
		1)
...
流量转发到的代理或端口。除ss协议外其它协议仅支持TCP转发！
...
EOF
			...
			is_number "$action" && [ "$action" -ge 1 ] && [ "$action" -le 4 ] && {
				...
				if [ "$server" ] && [ "$error_code" -le 0 ]; then
				...
				if [ "$server" ] && [ "$error_code" -le 0 ]; then
					...
					1)
						...
						if [ "$ipv4" ] && [ "$internet4" ]; then
							...
							ad1='fdfe:dcba:9876::1/126'
							ad2=6
						fi
						if sslocal --config /tmp/sslocal.json --protocol tun --tun-interface-name tun0 --tun-interface-address "$ad1" --daemonize --daemonize-pid /tmp/sslocal.pid --log-config /root/log4rs.yaml --log-without-time -U; then
							ip -${ad2} rule add fwmark 0x2233 table 123
							ip -${ad2} route add table 123 default dev tun0
							Proxy_Check "$ad2"
							if [ "$error_code" -le 0 ]; then
								if [ "$ad2" -eq 4 ]; then
									Start_traffic_forward_v4
									iptables -w -t mangle -R SHADOWSOCKS_v4 2 -m set --match-set traffic_forward_v4 dst -j MARK --set-mark 0x2233
									ipset add bypass_ips_v4 "$server"
								elif [ "$ad2" -eq 6 ]; then
									Start_traffic_forward_v6
									ip6tables -w -t mangle -R SHADOWSOCKS_v6 2 -m set --match-set traffic_forward_v6 dst -j MARK --set-mark 0x2233
									ipset add bypass_ips_v6 "$server"
								fi
							fi
		...
		10)
			if iptables -w -t mangle -C OUTPUT -o "$internet4" -j SHADOWSOCKS_v4 2>/dev/null; then
				...
				if [ "$(pgrep -F /tmp/sslocal.pid 2>/dev/null)" ]; then
					iptables -w -t mangle -I SHADOWSOCKS_v4 3 -p udp -m multiport --dport 80,443 -j MARK --set-mark 0x2233
				fi
			fi
			if ip6tables -w -t mangle -C OUTPUT -o "$internet6" -j SHADOWSOCKS_v6 2>/dev/null; then
				ip6tables -w -t mangle -R SHADOWSOCKS_v6 2 -p tcp -m multiport --dport 80,443 -j MARK --set-mark 0x2233
				if [ "$(pgrep -F /tmp/sslocal.pid 2>/dev/null)" ]; then
					ip6tables -w -t mangle -I SHADOWSOCKS_v6 3 -p udp -m multiport --dport 80,443 -j MARK --set-mark 0x2233
				fi
			fi
			;;
		11)
