...
Shadowsocks_info_input() {
	...
	cipher2022=0 cipher_no=0
	for i in "${Encryption_method_no[@]}"; do
		if [ "$i" = "$method" ]; then
			cipher_no=1
		fi
	done
	...
	#https://github.com/shadowsocks/shadowsocks-rust/issues/838
	if [ "$Dialog" = "enable" ]; then
		if [ "$cipher2022" -eq 0 ] && [ "$cipher_no" -eq 0 ]; then
			until [ "$password" ]; do
...
Add_user() {
	...
	if [ "$my_ipv4" ] || [ "$my_ipv6" ]; then
		if [ "$cipher_no" -eq 1 ]; then
			#https://github.com/shadowsocks/shadowsocks-rust/issues/838
			userinfo="$(echo -n "$method:" | base64 -w0 | sed 's/=//g; s/+/-/g; s/\//_/g')"
		#https://github.com/shadowsocks/shadowsocks-rust/commit/8156113402cf1017b0a805a1a87a2e57e1b219de 明文用户信息安卓客户端目前没支持
		elif [ "$cipher2022" -eq 1 ]; then
			userinfo="$(echo -n "$method:$password")"
		else
...
Upload_users() {
	if [ -s $HOME_DIR/port.list ]; then
		...
		while IFS= read -r line || [ -n "$line" ]; do
			...
			if is_number "$server_port" && is_number "$total" && [ -z "$using" ] && [ "$method" ]; then
...
EOF
			...
			is_number "$action" && [ "$action" -ge 1 ] && [ "$action" -le 4 ] && {
				...
				if [ "$server" ] && [ "$error_code" -eq 0 ]; then
